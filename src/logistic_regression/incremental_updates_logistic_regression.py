...
def prepare_term_1_2_batch2(beta, theta_list, grad_list, run_rc1, X, X_Y_prod, w_seq, b_seq, dim, batch_size, max_epoch, mini_batch_epoch, cut_off_epoch, curr_rand_ids_multi_super_iterations):
...
#     res = torch.mm(torch.t(w_seq), X_product).view(w_dim[1], dim[1], dim[1])
    ...
    for t in range(max_epoch):
        ...
        curr_rand_ids = curr_rand_ids_multi_super_iterations[t]
        
        curr_w_seq_end_id = (t+1)*dim[0]
        
        curr_w_seq_list = w_seq[t*dim[0]:curr_w_seq_end_id]
        
        curr_b_seq_list = b_seq[t*dim[0]:curr_w_seq_end_id]
...
#             batch_id = i%(batch_num)
            ...
            if t*dim[0] + i+ batch_size < w_dim[0]:
                ...
                curr_X = X[curr_rand_ids[i:i + batch_size]]
                

                curr_w_seq = curr_w_seq_list[curr_rand_ids[i:i+batch_size]]
                
#                 print(i*batch_size, (i + 1)*batch_size)
#                 print(curr_w_seq.shape)
                
                
                res1 = curr_X*curr_w_seq.view(curr_w_seq.shape[0], -1)

                curr_res = torch.mm(torch.t(curr_X), res1)
                
                
                
                
                curr_X_Y_prod = X_Y_prod[curr_rand_ids[i:i + batch_size]]

                curr_b_seq = curr_b_seq_list[curr_rand_ids[i:i+batch_size]]
                
                
                curr_X_Y_prod = curr_X_Y_prod.view(curr_X_Y_prod.shape[0], dim[1])
                
                curr_res2 = torch.mm(torch.t(curr_X_Y_prod), curr_b_seq.view(-1,1)).view(dim[1])
                
                
                origin_grad_this_epoch = grad_list[num]
                
#             if epoch > 0:
                computed_origin_grad_this_epoch =  (0 - (torch.mm((curr_res), theta_list[num]) + (curr_res2.view(-1,1))))/(end_id - i) + beta*theta_list[num]
            #             else:
#                 computed_origin_grad_this_epoch =  (0 - (torch.mm((term1_list[epoch]), origin_theta_this_epoch) + (full_term2.view(-1,1))))/(end_id - i) + beta*theta
                ...
                prod = torch.mm(curr_X_Y_prod.view(-1, dim[1]), theta_list[num])
#             
...
#                 offset_batch = offsets_this_super_iteration[random_ids[i:end_id]]
                ...
                delta = prod.view(-1)*curr_w_seq.view(-1) + curr_b_seq.view(-1) - (non_linear_function(prod)).view(-1)
                ...
                if run_rc1:
                    res3 = check_and_convert_to_sparse_tensor(curr_res)
                
                    del res1
                                    
                    res.append(res3)#torch.mm(torch.t(X), res1)
...
#                 curr_X_prod = curr_X.view(curr_X.shape[0], dim[1]*dim[1])
                
#                 res[num] = curr_res#torch.mm(torch.t(curr_X_prod), curr_w_seq).view(dim[1], dim[1])
                
            else:
                
                curr_X = X[curr_rand_ids[i:end_id]]
                
                curr_w_seq = curr_w_seq_list[curr_rand_ids[i:end_id]]
                res1 = curr_X*curr_w_seq.view(curr_w_seq.shape[0], -1)
...
#                 curr_X_prod = curr_X_prod.view(curr_X_prod.shape[0], dim[1]*dim[1])
                ...
                curr_X_Y_prod = X_Y_prod[curr_rand_ids[i:end_id]]

                curr_b_seq = curr_b_seq_list[curr_rand_ids[i:end_id]]
...
def prepare_term_1_batch2(run_rc1, X, w_seq, dim, batch_size, max_epoch, mini_batch_epoch, cut_off_epoch, curr_rand_ids_multi_super_iterations, mini_epochs_per_super_iteration):
...
#                     res3 = check_and_convert_to_sparse_tensor(curr_res)
                    ...
                    res3 = prepare_term_1_batch2_sparse(curr_X, curr_w_seq)
                
#                     del res1

                    print(res3.getnnz())
                    
                    res3 = scipy.sparse.csr_matrix(res3)
                    
#                     scipy.sparse.linalg.svds(res3)
...
#                     prepare_term_1_batch2_theta_sparse(curr_X, batch_X_multi_theta, batch_weights)
                    res1 = curr_X*curr_w_seq.view(curr_w_seq.shape[0], -1)

                    curr_res = torch.mm(torch.t(curr_X), res1)
                    
                    sub_u, sub_v = compute_single_svd(num, curr_res, batch_size)
            
                    np.save(directory + '/u_' + str(num), sub_u)
            
                    np.save(directory + '/v_' + str(num), sub_v)
                    
                    
                    if num >= cut_off_epoch - mini_epochs_per_super_iteration:
                        avg_term1 = avg_term1 + curr_res
...
#     num = 0
    ...
    cut_off_super_iterations = int(cut_off_epoch/min_batch_num_per_epoch)
    
    w_seq = w_seq[0:(cut_off_super_iterations*dim[0] + (cut_off_epoch - cut_off_super_iterations*min_batch_num_per_epoch)*batch_size)]
    
    b_seq = b_seq[0:(cut_off_super_iterations*dim[0] + (cut_off_epoch - cut_off_super_iterations*min_batch_num_per_epoch)*batch_size)]
...
#         res[ids[ids < cut_off_epoch]] = (term1[ids[ids < cut_off_epoch]] - curr_term1)/(end_id - i)
#         
#         res2[ids[ids < cut_off_epoch]] = (term2[ids[ids < cut_off_epoch]] - curr_term2)/(end_id - i)
...
#                 curr_B = alpha*res2
        ...
        print('term1::', term1[ids[ids < cut_off_epoch]][0])
          
        print('term2::', term2[ids[ids < cut_off_epoch]][0])
          
        print('curr_term1::', curr_term1[0])
                
        print('curr_term2::', curr_term2[0])
...
def prepare_term_1_batch2_delta3(random_ids_multi_super_iterations, alpha, beta, term1, term2, delta_X_product, delta_X_Y_prod, w_seq, b_seq, dim, batch_num, batch_size, delta_data_ids, mini_batch_epoch, max_epoch, cut_off_epoch, theta):
...
#     num = 0
    ...
    cut_off_super_iteration = int((w_seq.shape[0]-1)/dim[0])+1
...
#     '''T, n'''
#     
#     shuffled_noise_data_ids_index = torch.sum(matched_ids, 2)
#     
#     '''T, |delta_x|'''
#     shuffled_noise_data_ids = torch.nonzero(shuffled_noise_data_ids_index).view(-1, delta_data_ids.shape[0]).type(torch.IntTensor)
#     
#     '''T, 1 -> T, |delta_X|'''
#     
#     shuffled_noise_data_ids = shuffled_noise_data_ids + (torch.tensor(list(range(cut_off_super_iteration)))*dim[0]).view(-1, 1)
#     
# 

#     
# #     curr_origin_delta_ids = delta_data_ids + (torch.tensor(list(range(cut_off_super_iteration)))*dim[0]).view(-1, 1)
#     
#     curr_origin_delta_ids = shuffled_noise_data_ids.view(-1)
    ...
    curr_w_seq = w_seq[all_noise_data_ids].view(-1, delta_data_ids.view(-1).shape[0])
    
    curr_b_seq = b_seq[all_noise_data_ids].view(-1, delta_data_ids.view(-1).shape[0])
    
    '''[|delta_X|, m^2, 1] [|delta_x|, 1, T] -> |delta_x|, m^2, T'''
    '''delta_X_product: origin_ids'''
    
    delta_X_prod_w = torch.bmm(delta_X_product.view(delta_X_product.shape[0], -1, 1), (torch.t(curr_w_seq)).view(delta_X_product.shape[0], 1,-1))

    '''T, |delta_x|, m^2'''

    delta_X_prod_w = torch.transpose(torch.transpose(delta_X_prod_w, 1, 2), 0, 1)


    '''[|delta_X|, m, 1] [|delta_x|, 1, T] -> |delta_x|, m, T'''

    delta_X_Y_prod_b = torch.bmm(delta_X_Y_prod.view(delta_X_Y_prod.shape[0], -1, 1), (torch.t(curr_b_seq)).view(delta_X_Y_prod.shape[0], 1,-1))
    
    '''T, |delta_x|, m'''
    
    delta_X_Y_prod_b = torch.transpose(torch.transpose(delta_X_Y_prod_b, 1, 2), 0, 1)
    '''output: [m^2,t] = [m^2, T/B]'''
...
#     for T in range(delta_X_prod_w.shape[2]):

    
    
    all_data_ids = torch.zeros([cut_off_super_iteration, dim[0]])
...
#         curr_matched_ids = matched_ids[i:end_id,:,:]
#         
#         
#         
# 
...
# 
#         '''T,|delta_X|'''
#         curr_matched_ids = torch.sum(curr_matched_ids, 0)
# 
#         curr_matched_ids = curr_matched_ids.type(torch.DoubleTensor)
#         
#         '''T, 1'''
#         
#         total_curr_matched_ids = torch.sum(curr_matched_ids, 1)

        '''[T, |delta_x|, m^2]*[T, |delta_x|, 1] -> [T, |delta_x|, m^2] -> [T, m^2]'''
        
#         curr_term1 = torch.sum(delta_X_prod_w*curr_matched_ids.view(cut_off_super_iteration, -1, 1), 1)
        
        selected_delta_X_prod_w = delta_X_prod_w[curr_nonzero_ids_this_batch[:,0], curr_nonzero_ids_this_batch[:, 1]]
        
        curr_term1 = (torch.sum(selected_delta_X_prod_w.view(-1, dim[1]*dim[1], 1)*curr_nonzero_ids_count_this_batch.type(torch.DoubleTensor).view(-1, 1, cut_off_super_iteration), 0)).view(dim[1]*dim[1], cut_off_super_iteration)
        
        
        selected_delta_X_Y_prod_b = delta_X_Y_prod_b[curr_nonzero_ids_this_batch[:,0], curr_nonzero_ids_this_batch[:, 1]]
        
        curr_term2 = (torch.sum(selected_delta_X_Y_prod_b.view(-1, dim[1], 1)*curr_nonzero_ids_count_this_batch.type(torch.DoubleTensor).view(-1, 1, cut_off_super_iteration), 0)).view(dim[1], cut_off_super_iteration)


        curr_term1 = torch.t(curr_term1)
        
        curr_term2 = torch.t(curr_term2)
        '''[T, |delta_x|, m]*[T, |delta_x|, 1] -> [T, |delta_x|, m] -> [T, m]'''
#         curr_term2 = torch.sum(delta_X_Y_prod_b*curr_matched_ids.view(cut_off_super_iteration, -1, 1), 1)

        if int(i /batch_size) >= offset_mini_epochs:
            curr_term1 = curr_term1[0:-1]
            curr_term2 = curr_term2[0:-1]
            total_curr_matched_ids = total_curr_matched_ids[0:-1]
#         for T in range(cut_off_super_iteration):
#             curr_matched_ids_this_iteration = torch.nonzero(curr_matched_ids[T])
#             
#             delta_X_prod_w
...
#         
#         print(total_curr_matched_ids.shape)    
#         
#         print(term2[ids[ids < cut_off_epoch]].shape)
#         
#         print(offset_mini_epochs)
        
        res[ids[ids < cut_off_epoch]] = (term1[ids[ids < cut_off_epoch]] - curr_term1.view(-1, dim[1], dim[1]))/(end_id - i - total_curr_matched_ids.view(-1,1,1))
        
        res2[ids[ids < cut_off_epoch]] = (term2[ids[ids < cut_off_epoch]] - curr_term2).view(-1,dim[1])/(end_id - i - total_curr_matched_ids.view(-1,1))
...
#         '''T, |delta_X|'''
# 
#         curr_matched_ids = torch.nonzero(curr_matched_ids)
#             
#         
#         
#         curr_data_ids = cut_off_super_iteration.view(-1,1) + torch.tensor(list(range(i, end_id))).view(1,-1)
#         
#         all_data_ids[curr_data_ids.view(-1)] = 1
#         
#         
#         
#         
#         satisfiable_delta_ids = torch.nonzero(((i<= delta_data_ids)*(delta_data_ids < end_id)).view(-1))
#         
#         if satisfiable_delta_ids.shape[0] > 0:
#         
#             '''m^2, 1'''
#         
#             curr_term1 = torch.sum(delta_X_prod_w[satisfiable_delta_ids,:, T], 0).view(dim[1], dim[1])
#             
#             curr_term2 = torch.sum(delta_X_Y_prod_b[satisfiable_delta_ids, :, T], 0).view(-1)
#             
#             res[t] = (term1[t] - curr_term1)/(end_id - i - satisfiable_delta_ids.shape[0])
#             
#             res2[t] = (term2[t] - curr_term2)/(end_id - i - satisfiable_delta_ids.shape[0])
#             
#         else:
#             res[t] = (term1[t] - 0)/(end_id - i)
#             
#             res2[t] = (term2[t] - 0)/(end_id - i)
#             
#             
#         curr_A = (1 - alpha*beta)*torch.eye(dim[1], dtype = torch.double) + alpha*res[t]
#             
#         curr_B = alpha*res2[t]
#         
#         t += 1
#         
#         
#         if cut_off_epoch - t <= int((dim[0]-1)/batch_size) + 1:
#             last_A = torch.mm(last_A, curr_A)
#         
#             last_B = torch.mm(curr_A, last_B) + curr_B.view(-1,1)
#         
#         A = torch.mm(A, curr_A)
#         
#         B = torch.mm(curr_A, B) + curr_B.view(-1,1)
...
#             print(B)
#             
#             print('term1::', term1[t-1])
#             
#             print('term2::', term2[t-1])
#             
#             print('curr_term1::', curr_term1)
#             
#             print('curr_term2::', curr_term2)
#             
#             print(torch.mm(A, theta) + B)
        
    
    
#     offset_w_seq = w_seq[cut_off_super_iteration*dim[0]:w_seq.shape[0]]
#     
#     offset_b_seq = b_seq[cut_off_super_iteration*dim[0]:w_seq.shape[0]]
#     
#     if offset_w_seq.shape[0] > 0:
#     
#         for i in range(0, dim[0], batch_size):
#                 
#             end_id = i + batch_size
#             
#             if i >= offset_w_seq.shape[0]:
#                 break
#             
#             
#             if end_id > dim[0]:
...
#                 '''[|delta_X, 1|]'''
#                 
#                 curr_w_seq = offset_w_seq[delta_data_ids[satisfiable_delta_ids]].view(-1,1)
#             
#                 curr_b_seq = offset_b_seq[delta_data_ids[satisfiable_delta_ids]].view(-1,1)
# 
#                 '''[|delta_X|, m^2, 1] [|delta_x|, 1, 1] -> |delta_x|, m^2, 1'''
#     
#                 delta_X_prod_w = delta_X_product[satisfiable_delta_ids].view(satisfiable_delta_ids.shape[0], -1, 1)*curr_w_seq.view(satisfiable_delta_ids.shape[0], 1,-1)
#             
#                 '''[|delta_X|, m, 1] [|delta_x|, 1, T] -> |delta_x|, m, 1'''
#             
#                 delta_X_Y_prod_b = delta_X_Y_prod[satisfiable_delta_ids].view(satisfiable_delta_ids.shape[0], -1, 1)*curr_b_seq.view(satisfiable_delta_ids.shape[0], 1,-1)
# 
#                 
#                 
#                 curr_term1 = torch.sum(delta_X_prod_w, 0).view(dim[1], dim[1])
#                 
#                 curr_term2 = torch.sum(delta_X_Y_prod_b, 0).view(-1)
#                 
#                 res[t] = (term1[t] - curr_term1)/(end_id - i - satisfiable_delta_ids.shape[0])
#                 
#                 res2[t] = (term2[t] - curr_term2)/(end_id - i - satisfiable_delta_ids.shape[0])
...
def prepare_term_2_batch2(run_rc1, X_Y_prod, b_seq, dim, max_epoch, mini_batch_epoch, batch_size, cut_off_epoch, curr_rand_ids_multi_super_iterations):
...
#     batch_num = int(dim[0]/batch_size)
    ...
    for t in range(max_epoch):
        ...
        curr_b_seq_list = b_seq[t*dim[0]:(t+1)*dim[0]]
...
#             batch_id = i%(batch_num)
            ...
            if t*dim[0] + i + batch_size < b_dim[0]:
                ...
                if run_rc1:
                    res3 = compute_sub_term_2_by_epoch_sparse(curr_X_Y_prod, curr_b_seq)
                    ...
                    res.append(torch.mm(torch.t(curr_X_Y_prod), curr_b_seq.view(-1, 1)).view(dim[1]))
