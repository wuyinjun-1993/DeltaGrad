...
def cal_approx_hessian_vec_prod0(S_k_list, Y_k_list, i, m, k, v_vec, period):
    ...
    ids = period*torch.tensor(range(period_num + 1))
...
#         expected_para = get_devectorized_parameters(get_all_vectorized_parameters(expected_para) - alpha*get_all_vectorized_parameters(get_all_gradient(model)), input_dim, hidden_dims, output_dim)
        ...
        if i< 5 or use_standard_way == True:
            
            init_model(model, para)
            
            compute_derivative_one_more_step(model, error, update_X, update_Y, beta)
...
#             init_model(model, para)
            ...
            compute_derivative_one_more_step(model, error, delta_X, delta_Y, beta)
...
#             delta_Y = Y[delta_ids]
            ...
            gradient_dual = get_all_vectorized_parameters(get_all_gradient(model))
