...
#     X = torch.load(git_ignore_folder+'noise_X')
    ...
    origin_X = torch.load(git_ignore_folder + 'X')
    
    origin_Y = torch.load(git_ignore_folder + 'Y')
...
#     b_delta_prod_list = torch.load(git_ignore_folder + 'b_delta_prod_list')
    ...
    w_list_all_epochs = torch.load(git_ignore_folder + 'w_seq')
     
    b_list_all_epochs = torch.load(git_ignore_folder + 'b_seq')
...
#     model_para_list = model_update_provenance_cp(alpha, origin_X, origin_Y, hessian_matrix, gradient_list, get_all_vectorized_parameters(list(model.parameters())), max_epoch, model, dim, w_list, b_list, input_dim, hidden_dims, output_dim, delta_data_ids)
    model_para_list = model_update_provenance_cp2(alpha, origin_X, origin_Y, max_epoch, model, dim, w_list_all_epochs, b_list_all_epochs, input_dim, hidden_dims, output_dim, selected_rows, exp_gradient_list_all_epochs, exp_para_list_all_epochs)
