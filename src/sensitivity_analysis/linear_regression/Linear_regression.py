...
def linear_regression(X, Y, dim, lr, tracking_or_not):
...
#     add_feature = bool(int(sys_args[8]))
    ...
    add_noise_or_not = bool(int(sys_args[8]))
    
    
    extend_dimesions = bool(int(sys_args[9]))
    ...
    if start:
        ...
        res1, epoch = compute_parameters(X, Y, dim, lr, False)
...
#         
    ...
    #     torch.save(noise_data_ids, git_ignore_folder + 'noise_data_ids')
        ...
        res2, epoch = compute_parameters(X, Y, dim, lr, True)
    ...
    #     X_Y_mult = X.mul(Y)
        ...
        capture_provenance(X, Y, alpha, beta)
