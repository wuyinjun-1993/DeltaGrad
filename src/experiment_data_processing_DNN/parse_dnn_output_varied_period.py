...
file_name = '../../scripts_general/mnist_lr_varied_deletion_rate'

# file_name = '../../scripts_general/mnist_dnn_deletion'
 
# output_file_suffix = 'lr_higgs'
output_file_suffix = 'mnist_deletion'
...
#     approach_list = list(results.keys())
    ...
    training_time_arr = np.zeros((deletion_rate_num, batch_size_num, deletion_rates_num, total_experiment_number))
    
    distance_arr = np.zeros((deletion_rate_num, batch_size_num, deletion_rates_num, total_experiment_number))
    
    test_accuracy_arr = np.zeros((deletion_rate_num, batch_size_num, deletion_rates_num, total_experiment_number), dtype = np.double)
    
    final_training_time_arr = np.zeros((deletion_rate_num, batch_size_num, total_experiment_number))
    
    final_distance_arr = np.zeros((deletion_rate_num, batch_size_num, total_experiment_number))
    
    final_test_accuracy_arr = np.zeros((deletion_rate_num, batch_size_num, deletion_rates_num, total_experiment_number), dtype = np.double)
    
#     final_test_accuracy_arr_diff = np.zeros((deletion_rate_num, batch_size_num, total_experiment_number - 2), dtype = np.double)
    
    final_test_accuracy_arr_var = np.zeros((deletion_rate_num, batch_size_num, total_experiment_number), dtype = np.double)
...
#                     print(this_res[curr_repetition][methods[p]])
    ...
    final_test_err_arr = np.mean(test_err_arr, 2)
    
    
    
    final_test_accuracy_arr_var = np.mean((test_accuracy_arr - final_test_accuracy_arr.reshape((deletion_rate_num, batch_size_num, 1, total_experiment_number)))**2, 2)
    
    
    final_test_err_arr_var = np.mean((test_err_arr - final_test_err_arr.reshape((deletion_rate_num, batch_size_num, 1, total_experiment_number)))**2, 2)
...
#     approach_list.insert(0, '')
    ...
    for i in range(len(deletion_rate_list)):
        ...
        convert2df_test_acc("batch_size", final_test_accuracy_arr[i], batch_size_list, final_label_list).to_excel(writer, sheet_name='test accuracy dr (' + str(deletion_rate_list[i]) + ")", index=False, float_format='%11.8f')
...
#         convert2df(final_test_accuracy_arr_diff[i], batch_size_list, methods[1:]).to_excel(writer, sheet_name='test accuracy diff dr (' + str(deletion_rate_list[i]) + ")", index=False)
    ...
    for i in range(len(range(batch_size_num))):
        ...
        convert2df_test_acc("deletion rate", final_test_accuracy_arr[:,i], deletion_rate_list, final_label_list).to_excel(writer, sheet_name='test accuracy bz (' + str(batch_size_list[i]) + ")", index=False, float_format='%11.8f')
...
#                 if len(results_curr_repe) == len(methods):
                if recorded_count >= total_experiment_number:
                    ...
                    print(results_all_repetition.keys())
                    ...
                    if len(results_all_repetition) == random_set_num:
                        ...
                        if len(results_curr_deletion) == batch_sizes:
