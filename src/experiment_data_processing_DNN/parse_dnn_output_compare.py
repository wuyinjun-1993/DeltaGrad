...
file_name = '../../scripts_general/covtype_lr_compare'
 
output_file_suffix = 'covtype_lr_compare'

headers = ["varied number of samples::"]
...
methods = ["baseline", "incremental updates 0", "incremental updates"]

time_labels = ["time_baseline::","time_provenance0::", "time_provenance::"]

distance_prefixs = ["tensor(", "absolute_error:: tensor(", "tensor("]
...
def convert2df(arrays, noise_rates, approach_lists):
    ...
    d['deletion rate'] = np.array(list(noise_rates))
...
def compute_confidence_interval(mean_values, variance_values, sample_size):
    ...
    upper_bound = mean_values + 2.567*np.sqrt(variance_values/sample_size)
    
    lower_bound = mean_values - 2.567*np.sqrt(variance_values/sample_size)
...
#     batch_size_list = list(results[deletion_rate_list[0]].keys())
    ...
    repetition_list = results[deletion_rate_list[0]].keys()
...
#     batch_size_num = len(batch_size_list)
    ...
    training_time_arr = np.zeros((deletion_rate_num, len(methods)))
    
    distance_arr = np.zeros((deletion_rate_num, len(methods)))
    
    test_accuracy_arr = np.zeros((deletion_rate_num, len(methods)), dtype = np.double)
    mem_usage_arr = np.zeros((deletion_rate_num, len(methods) - 1), dtype = np.double)
    
    final_training_time_arr = np.zeros((deletion_rate_num, len(methods)))
    
    final_distance_arr = np.zeros((deletion_rate_num, len(methods)))
    
    final_test_accuracy_arr = np.zeros((deletion_rate_num, len(methods)), dtype = np.double)
    
    final_test_accuracy_arr_diff = np.zeros((deletion_rate_num, len(methods) - 1), dtype = np.double)
    
    final_test_accuracy_arr_var = np.zeros((deletion_rate_num, len(methods)), dtype = np.double)
...
#     print(approach_list)
    for i in range(len(deletion_rate_list)):
        ...
        this_res = results[deletion_rate_list[i]]
...
#                     print(this_res[curr_repetition][methods[p]])
            ...
            if p > 0:
                mem_usage_arr[i][p - 1] = this_res[methods[p]][mem_usage_label]

            training_time_arr[i][p] = this_res[methods[p]][running_time_label]
            
            distance_arr[i][p] = this_res[methods[p]][distance_label]
            
            test_accuracy_arr[i][p] = this_res[methods[p]][accuracy_label]
...
#     
#     test_error_arr = 1 - test_accuracy_arr
#     
#     final_test_error_arr = np.mean(test_error_arr, 1)
#     
#     final_test_accuracy_arr_var = np.mean((test_error_arr - final_test_error_arr.reshape((deletion_rate_num, 1, len(methods))))**2, 1)
#     
#     
#     lower_bound, upper_bound = compute_confidence_interval(final_test_error_arr, final_test_accuracy_arr_var, 10000)
#     
...
#     for i in range(len(range(batch_size_num))):
    convert2df(training_time_arr, deletion_rate_list, methods).to_excel(writer, sheet_name='training time', index=False)
    convert2df(distance_arr, deletion_rate_list, methods).to_excel(writer, sheet_name='distance', index=False)
    convert2df(test_accuracy_arr, deletion_rate_list, methods).to_excel(writer, sheet_name='test accuracy', index=False)
    convert2df(mem_usage_arr, deletion_rate_list, methods[1:]).to_excel(writer, sheet_name='memory usage', index=False)
#     convert2df(final_test_accuracy_arr_var, deletion_rate_list, methods).to_excel(writer, sheet_name='test accuracy var', index=False)
#     convert2df(lower_bound, deletion_rate_list, methods).to_excel(writer, sheet_name='test accuracy CI lower', index=False)
#     convert2df(upper_bound, deletion_rate_list, methods).to_excel(writer, sheet_name='test accuracy CI upper', index=False)
#     
#     convert2df(final_test_accuracy_arr_diff, deletion_rate_list, methods[1:]).to_excel(writer, sheet_name='test accuracy diff', index=False)
...
#                 continue
            ...
            if state == 3 and line.startswith(mem_usage_label):
                mem_usage = float(line[len(mem_usage_label):].strip())
                results_curr_repe[methods[curr_method_id]] = {}
                
                results_curr_repe[methods[curr_method_id]][mem_usage_label]= mem_usage/(1e9)
...
#                 print("curr_mathod_id::", curr_method_id)
            ...
            if state == 5 and line.startswith(distance_prefixs[curr_method_id]):
                distance = get_dstance_value(distance_prefixs[curr_method_id], line)
...
#                             print("deletion_rate::", deletion_rate)
                    ...
                    results[deletion_rate] = results_curr_repe.copy()
                    results_curr_repe.clear()
