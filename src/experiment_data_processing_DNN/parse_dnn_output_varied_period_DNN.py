...
# file_name = '../../scripts_general/rcv1_lr_varied_batch_period'

file_name = '../../scripts_general/mnist_dnn_add'
 
# output_file_suffix = 'lr_rcv1'

output_file_suffix = 'DNN_add'
...
method_headers = ["python3 benchmark_exp.py", "baseline::", "period::"]

time_labels = ["training_time::","time_baseline::", "time_provenance::"]
...
#     batch_size_num = len(batch_size_list)
    ...
    training_time_arr = np.zeros((deletion_rate_num, deletion_rates_num, total_experiment_number))
    
    distance_arr = np.zeros((deletion_rate_num, deletion_rates_num, total_experiment_number))
    
    test_accuracy_arr = np.zeros((deletion_rate_num, deletion_rates_num, total_experiment_number), dtype = np.double)
    
    final_training_time_arr = np.zeros((deletion_rate_num, total_experiment_number))
    
    final_distance_arr = np.zeros((deletion_rate_num, total_experiment_number))
    
    final_test_accuracy_arr = np.zeros((deletion_rate_num, deletion_rates_num, total_experiment_number), dtype = np.double)
    
#     final_test_accuracy_arr_diff = np.zeros((deletion_rate_num, batch_size_num, total_experiment_number - 2), dtype = np.double)
    
    final_test_accuracy_arr_var = np.zeros((deletion_rate_num, total_experiment_number), dtype = np.double)
...
#             print(deletion_rate_list[i], curr_batch_size, this_res)
        ...
        for k in range(deletion_rates_num):
            ...
            curr_repetition = list(curr_results.keys())[k]
...
#                     print(this_res[curr_repetition][methods[p]])
                ...
                if len(final_label_list) < total_experiment_number:
                    ...
                    if p >= 2:
                        ...
                        print(curr_results[curr_repetition][label])
                ...
                print(curr_results[curr_repetition])
    ...
    final_test_accuracy_arr_var = np.mean((test_accuracy_arr - final_test_accuracy_arr.reshape((deletion_rate_num, 1, total_experiment_number)))**2, 1)
    
    
    final_test_err_arr_var = np.mean((test_err_arr - final_test_err_arr.reshape((deletion_rate_num, 1, total_experiment_number)))**2, 1)
...
#     for i in range(len(deletion_rate_list)):
#         convert2df("batch_size", final_training_time_arr[i], batch_size_list, final_label_list).to_excel(writer, sheet_name='training time dr (' + str(deletion_rate_list[i]) + ")", index=False)
...
#     for i in range(len(range(batch_size_num))):
    convert2df("deletion rate", final_training_time_arr, deletion_rate_list, final_label_list).to_excel(writer, sheet_name='training time', index=False)
    convert2df("deletion rate", final_distance_arr, deletion_rate_list, final_label_list).to_excel(writer, sheet_name='distance', index=False)
    convert2df("deletion rate", final_test_accuracy_arr, deletion_rate_list, final_label_list).to_excel(writer, sheet_name='test accuracy', index=False)
    convert2df("deletion rate", final_test_accuracy_arr_var, deletion_rate_list, final_label_list).to_excel(writer, sheet_name='test accuracy var', index=False)
    convert2df("deletion rate", lower_bound, deletion_rate_list, final_label_list).to_excel(writer, sheet_name='test accuracy CI lower', index=False)
    convert2df("deletion rate", upper_bound, deletion_rate_list, final_label_list).to_excel(writer, sheet_name='test accuracy CI upper', index=False)
...
#                             print("deletion_rate::", deletion_rate)
                            ...
                            results[deletion_rate] = results_all_repetition.copy()
                            results_all_repetition.clear()
