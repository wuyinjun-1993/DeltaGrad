...
def compute_confidence_interval(mean_values, variance_values, sample_size):
    ...
    upper_bound = 2.567*np.sqrt(variance_values/sample_size)
    
    lower_bound = 2.567*np.sqrt(variance_values/sample_size)
...
#     batch_size_num = len(batch_size_list)
    ...
    training_time_arr = np.zeros((deletion_rate_num, deletion_rates_num, len(methods)))
    
    distance_arr = np.zeros((deletion_rate_num, deletion_rates_num, len(methods)))
    
    test_accuracy_arr = np.zeros((deletion_rate_num, deletion_rates_num, len(methods)), dtype = np.double)
    
    final_training_time_arr = np.zeros((deletion_rate_num, len(methods)))
    
    final_distance_arr = np.zeros((deletion_rate_num, len(methods)))
    
    final_test_accuracy_arr = np.zeros((deletion_rate_num, deletion_rates_num, len(methods)), dtype = np.double)
...
#                     print(this_res[curr_repetition][methods[p]])
                ...
                training_time_arr[i][k][p] = this_res[curr_repetition][methods[p]][running_time_label]
                
                distance_arr[i][k][p] = this_res[curr_repetition][methods[p]][distance_label]
                
                test_accuracy_arr[i][k][p] = this_res[curr_repetition][methods[p]][accuracy_label]
...
#     for i in range(len(range(batch_size_num))):
    convert2df(final_training_time_arr, deletion_rate_list, methods).to_excel(writer, sheet_name='training time', index=False)
    convert2df(final_distance_arr, deletion_rate_list, methods).to_excel(writer, sheet_name='distance', index=False)
    convert2df(final_test_accuracy_arr, deletion_rate_list, methods).to_excel(writer, sheet_name='test accuracy', index=False)
