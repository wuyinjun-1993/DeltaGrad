...
def estimate_gradients(gradient_list_all_epochs, para_list_all_epochs,  model, random_ids_all_epochs, batch_size, X, Y, criterion, optimizer, delta):
    
    id_list = torch.load(git_ignore_folder + 'delta_data_id_index_list')
...
#         para_id = 10
        ...
        exp_gradient_list_all_epochs = torch.load(git_ignore_folder + 'expected_gradient_list_all_epochs_' + str(id))
      
        exp_para_list_all_epochs = torch.load(git_ignore_folder + 'expected_para_list_all_epochs_' + str(id))
        ...
        U, S, V =torch.svd(delta_para_tensor)
        ...
        if kk > 0:
            coeff_x = torch.mm(V_list[kk-1], torch.t(V_list[kk]))
            
            residual1 = torch.norm(torch.mm(coeff_x, V_list[kk]) - V_list[kk-1])
            
            print("residual1::", residual1)
            
            
            coeff_x = torch.mm(U_list[kk-1], torch.t(U_list[kk]))
            
            residual2 = torch.norm(torch.mm(coeff_x, U_list[kk]) - U_list[kk-1])
    ...
    exp_gradient_list_all_epochs = torch.load(git_ignore_folder + 'expected_gradient_list_all_epochs_' + str(id_list[-5].item()))
      
    exp_para_list_all_epochs = torch.load(git_ignore_folder + 'expected_para_list_all_epochs_' + str(id_list[-5].item()))
    ...
    for para_id in para_ids:
        ...
        perturbed_para_all = torch.zeros([upper_bound, total_shape_size], dtype = torch.double)
...
#             curr_shape = first_para[i].shape
            ...
            perturbed_para = get_devectorized_parameters(get_all_vectorized_parameters(first_para) + delta*V_list[0][:,i].view(1,-1), full_shape_list, shape_list)
...
#         
        ...
        sub_s = S[0:grad_dim_count]
         
     
        sub_u = U[:,0:grad_dim_count]
          
         
          
        sub_v = V[:,0:grad_dim_count]
#         
...
#         
        ...
        res_delta_grad2 = get_exp_grad_delta(para_delta, perturbed_para_all, perturbed_grad_all)
...
#     print("learning rate::", alpha)
    ...
    estimate_gradients(gradient_list_all_epochs, para_list_all_epochs, model, random_ids_multi_super_iterations, batch_size, dataset_train.data, dataset_train.labels, criterion, optimizer, delta_data_ids.shape[0]*1.0*0.01/dataset_train.data.shape[0])
