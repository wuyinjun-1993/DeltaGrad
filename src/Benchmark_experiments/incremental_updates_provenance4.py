...
def compute_gradient_diff(gradient_list_all_epochs):
    
    
    last_grad = gradient_list_all_epochs[0]
    
    for i in range(len(gradient_list_all_epochs) - 1):
        
        print("iteration", (i+1))
        
        compute_model_para_diff(list(last_grad), list(gradient_list_all_epochs[i+1]))
        
        
        last_grad = gradient_list_all_epochs[i+1]
...
#     model_para_list = model_update_provenance_test(None, None, exp_gradient_list_all_epochs, exp_para_list_all_epochs, origin_X, origin_Y, model, S_k_list, Y_k_list, gradient_list_all_epochs, para_list_all_epochs, max_epoch, delta_data_ids, input_dim, hidden_dims, output_dim, 50, alpha, beta, selected_rows, error)
    ...
    '''max_period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, gradient_list_all_epochs, para_list_all_epochs, epoch, delta_ids, m, learning_rate_all_epochs, beta, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device'''
