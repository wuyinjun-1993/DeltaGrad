...
#     torch.backends.cudnn.enabled = False
    ...
    print("training set size::", dataset_train.data.shape)
...
#     model, count = model_update_standard_lib_stochastic(batch_size, max_epoch, update_X, update_Y, alpha, error, model)
    t1 = time.time()
    all_para_list, summed_delta_para, count, exp_para_list_all_epochs, exp_gradient_list_all_epochs = model_update_standard_lib_jackknife(origin_model, max_epoch, dataset_train, model, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, learning_rate_all_epochs, criterion, optimizer, para_list_all_epochs, gradient_list_all_epochs, is_GPU, device, record_params)
...
#     print_model_para(model)
    ...
    torch.save(summed_delta_para, git_ignore_folder + 'summed_delta_para')
    
    torch.save(all_para_list, git_ignore_folder + 'all_para_list')
...
#     test(model, dataset_train, batch_size, criterion, len(dataset_train), is_GPU, device)
#     test(model, data_test_loader, criterion, len(dataset_train), is_GPU, device)
