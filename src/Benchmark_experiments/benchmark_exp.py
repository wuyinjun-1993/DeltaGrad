...
def get_subset_data_per_epoch_diff(curr_rand_ids, delta_id_set):
...
#     return curr_rand_ids[ids[:,0]]
    ...
    curr_matched_ids = torch.tensor(list(curr_rand_id_set.difference(delta_id_set)))
...
def model_update_standard_lib_jackknife(origin_model, num_epochs, dataset_train, model, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, learning_rate_all_epochs, criterion, optimizer, para_list_all_epochs, gradient_list_all_epochs, is_GPU, device, record_params):
#     for epoch in range(num_epochs):
#     loss = np.infty
...
#     random.seed(random_seed)
    ...
    summed_delta_para = None
    
    
    
#     for r in range(len(dataset_train)):
    for r in range(20):
        
        count = 0

#         delta_data_set = set([r])
        
        random_ids_list_all_epochs = []
        
        delta_data_ids = torch.tensor([r])
        
        print("removed sample:", r)
    ...
    #             print(i, len(random_ids_list))
                ...
                curr_matched_ids = get_remaining_subset_data_per_epoch(random_ids[j:end_id], random_ids[all_indexes[id_start:id_end]])
                
    #             print(k,j, curr_matched_ids.shape[0])
...
#                 compute_model_para_diff(gradient_list_all_epochs[count], list(get_model_gradient(model)))
    ...
    #             update_and_zero_model_gradient(model,learning_rate)
        ...
        if summed_delta_para is None:
            summed_delta_para = get_all_vectorized_parameters(model.get_all_parameters())
        else:
            summed_delta_para += get_all_vectorized_parameters(model.get_all_parameters())
            
            
        all_para_list.append(model.get_all_parameters())
#         all_para_list.append(get_all_vectorized_parameters(model.get_all_parameters()))
        print('parameter comparison::')
         
#         compute_model_para_diff(list(model.parameters()), list(origin_model.parameters()))
#         
#         print("here")
...
#     print("count3::", count3)
    ...
    return all_para_list, summed_delta_para, count, exp_para_list_all_epochs, exp_gradient_list_all_epochs
    

def model_update_standard_lib(num_epochs, dataset_train, model, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, delta_data_ids, batch_size, learning_rate_all_epochs, criterion, optimizer, para_list_all_epochs, gradient_list_all_epochs, is_GPU, device, record_params):
    count = 0
#     for epoch in range(num_epochs):
#     loss = np.infty
...
#     count3 = 0
    ...
    delta_data_set = set(delta_data_ids.view(-1).tolist())
...
#             
# #             print(len(curr_rand_ids), len(delta_data_ids))
#             
#             
#             t7 = time.time()
#             curr_matched_ids = get_remaining_subset_data_per_epoch(curr_rand_ids, delta_data_set)
#             t8 = time.time()
#     
#             overhead3 += (t8 - t7)
#             
...
def model_update_standard_lib_multi(num_epochs, dataset_train, model, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, delta_data_ids, batch_size, learning_rate_all_epochs, criterion, optimizer, para_list_all_epochs, gradient_list_all_epochs, is_GPU, device, record_params):
...
#     init_para = list(model.parameters())
...
#     data_train_loader.shuffle = False
    ...
    prev_random_ids_list_all_epochs = None
    ...
    for r in range(len(delta_data_ids)):
        ...
        delta_id = delta_data_ids[r:r+1]
    
        curr_delta_id = torch.tensor(delta_id)
        
        random_ids_list_all_epochs = []
        
        curr_exp_gradient_list_all_epochs = []
...
#             for k in range(len(random_ids_multi_super_iterations)):
            for k in range(num_epochs):
                ...
                if curr_delta_id.shape[0] > 1:
                    all_indexes = np.sort(sort_idx[curr_delta_id])
                else:
                    all_indexes = torch.tensor([sort_idx[curr_delta_id]])
    ...
    #                 if random_ids[all_indexes[id_start:id_end]].shape[0]> 0:
    #                     print("here")
                    removed_ids = random_ids[all_indexes[id_start:id_end]]
...
#             for k in range(len(prev_random_ids_list_all_epochs)):
            for k in range(num_epochs):
                
                all_prev_random_ids = prev_random_ids_list_all_epochs[k]
...
#                 for j in range(0, len(dataset_train), batch_size):
                j = 0
                for p in range(len(all_prev_random_ids)):
                    prev_random_ids = all_prev_random_ids[p]
    ...
    #                     print("here")
                    ...
                    if removed_ids.shape[0] > 0:
#                         print(k, p)
                        curr_matched_ids = get_remaining_subset_data_per_epoch(prev_random_ids, removed_ids)
                    else:
                        curr_matched_ids = prev_random_ids
...
#                 random_ids = random_ids_multi_super_iterations[k]
#             
#                 prev_random_ids_list = prev_random_ids_list_all_epochs[k]
#                 
#                 sort_idx = sorted_ids_multi_super_iterations[k]#random_ids.numpy().argsort()
#                 
#                 if curr_delta_id.shape[0] > 1:
...
#                 j = 0
# 
#                 for p in range(len(prev_random_ids_list)):
#                     prev_random_ids = prev_random_ids_list[p]
#                 
#                 
#         #             print(i, len(random_ids_list))
#                     
...
#                     
#                     
#                     curr_matched_ids = get_remaining_subset_data_per_epoch(prev_random_ids, random_ids[all_indexes[id_start:id_end]])
#                     
#         #             print(k,j, curr_matched_ids.shape[0])
#                     
#                     random_ids_list.append(curr_matched_ids)
...
#                 if r > 0 and count == 133:
#                     print(curr_random_ids.shape[0])
#                     
#                     torch.save(curr_random_ids, git_ignore_folder + "tmp_curr_random_ids1")
#                     
#                 if r > 0 and count == 134:
#                     print(curr_random_ids.shape[0])
#                     
#                     torch.save(curr_random_ids, git_ignore_folder + "tmp_curr_random_ids2")
...
#                 exp_grad = gradient_list_all_epochs[count]
#                 
#                 exp_para = para_list_all_epochs[count]
#                 
#                 print("para diff:")
# 
# 
#                 compute_model_para_diff(exp_para, list(model.parameters()))
#                 
#                 print("grad diff:")
#                 
#                 compute_model_para_diff(exp_grad, model.get_all_gradient())
                
                
                if record_params:
                    append_gradient_list(curr_exp_gradient_list_all_epochs, None, curr_exp_para_list_all_epochs, model, batch_X, is_GPU, device)
    
                optimizer.step()
                
                count += 1
        
        res_para_list.append(get_all_vectorized_parameters1(model.parameters()).clone())
        ...
        if record_params:
            exp_gradient_list_all_epochs.append(curr_exp_gradient_list_all_epochs)
        ...
        prev_random_ids_list_all_epochs = random_ids_list_all_epochs
#         prev_random_ids_list_all_epochs = random_ids_list_all_epochs.copy()
...
#     print("count3::", count3)
    ...
    return model, count, exp_para_list_all_epochs, exp_gradient_list_all_epochs, res_para_list
...
#     for i, items in enumerate(data_test_loader):
    ...
    net.train()
    print('Test Avg. Loss: %lf, Accuracy: %lf' % (avg_loss.detach().cpu().item(), float(total_correct) / data_test_size))
...
def model_training(epoch, net, dataset_train, data_test_loader, data_train_size, data_test_size, optimizer, criterion, lr_scheduler, batch_size, is_GPU, device, lrs):
...
def model_training_lr_test(random_ids_multi_super_iterations, epoch, net, dataset_train, dataset_test, data_train_size, data_test_size, optimizer, criterion, lr_scheduler, batch_size, is_GPU, device, lrs):
#     global cur_batch_win
    net.train()
...
def get_all_vectorized_parameters0(para_list, full_shape_list, shape_list, res):
...
#         total_len = 0
#         
#         for j in range(len(full_shape_list[i])):
#             total_len *= full_shape_list[i][j]
...
#             tmp = curr_shape_list[0]
#              
#             curr_shape_list[0] = curr_shape_list[1]
#              
#             curr_shape_list[1] = tmp
            
            res[pos: pos+shape_list[i]] = para_list[i].view(-1)
            
#             param = torch.transpose(param, 0, 1)
            
#             if origin_param is not None:
#                 print(torch.norm(list(origin_param)[i] - param))
...
def compute_diff_vectorized_parameters2(para_list1, para_list2, vec_para_diff, shape_list, if_GPU, device):
#     res_list = []
...
def prepare_hessian_vec_prod0_4(S_k_list, Y_k_list, i, m, k, is_GPU, device):
...
def compute_approx_hessian_vector_prod_with_prepared_terms0(zero_mat_dim, curr_Y_k, curr_S_k, sigma_k, mat, v_vec, is_GPU, device):
...
#     old_para = []
    ...
    if gradient_dual is not None:
        ...
        for i in range(len(para)):
            ...
            if is_GPU:
                hessian_para_prod[i] += (grad_list[i].to(device) + beta*para_list[i].to(device))
            else:
                hessian_para_prod[i] += (grad_list[i] + beta*para_list[i])
...
#             print(hessian_para_prod.shape)
            if is_GPU:
                gradients = (hessian_para_prod[i]*size1 - (gradient_dual[i] + beta*para[i])*size2)/(size1 - size2)
            else:
                gradients = (hessian_para_prod[i]*size1 - (gradient_dual[i].to('cpu') + beta*para[i])*size2)/(size1 - size2)
#             para[i] *= (1-alpha*beta)
...
def compute_grad_final4(vec_para, hessian_para_prod, gradient_dual, grad_list_tensor, para_list_tensor, size1, size2, alpha, beta, is_GPU, device):
    
    gradients = None
    
    exp_gradient_full = None
...
#         gradients = (hessian_para_prod[i]*size1 - (gradient_dual[i].to('cpu') + beta*para[i])*size2)/(size1 - size2)
        ...
        exp_gradient_full =  gradients - beta*vec_para
...
#         exp_gradient_full = hessian_para_prod - beta*vec_para
...
#         exp_gradient_full =  hessian_para_prod - beta*vec_para
    ...
    delta_para = vec_para - para_list_tensor
    ...
    if torch.norm(delta_para) > torch.norm(delta_grad):
        return True, gradients, exp_gradient_full

    else:
        return False, gradients, exp_gradient_full
...
def model_update_provenance_test3(max_epoch, period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, gradient_list_all_epochs_tensor, para_list_all_epochs_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, cached_size, delta_ids, m, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device):
...
#                     print("para diff::")
#  
#                     compute_model_para_diff(para, exp_para_list_all_epochs[i])
#                       
#                     print("gradient diff::") 
#                      
#                     compute_model_para_diff(get_devectorized_parameters(expect_gradients, full_shape_list, shape_list), exp_gradient_list_all_epochs[i])

                    
#                     res_para_list.append(curr_para)
#                     
#                     res_grad_list.append(expect_gradients + regularization_coeff*curr_para)
...
#                     torch.save(batch_delta_X, git_ignore_folder + 'tmp_delat_x')
#                     
#                     torch.save(batch_delta_Y, git_ignore_folder + 'tmp_delat_y')
                    
                    gradient_dual = model.get_all_gradient()
                    
#                     torch.save(gradient_dual, git_ignore_folder + 'grad_dual')
...
#                     print("gradient diff::") 
#                     
#                     exp_grad_list = get_all_vectorized_parameters1(exp_gradient_list_all_epochs[i]) + regularization_coeff*get_all_vectorized_parameters1(exp_para_list_all_epochs[i])
#                      
#                     compute_model_para_diff(get_devectorized_parameters(final_gradient_list, full_shape_list, shape_list), get_devectorized_parameters(exp_grad_list, full_shape_list, shape_list))
...
#                     res_para_list.append(get_all_vectorized_parameters1(para))
#                     
#                     res_grad_list.append(final_gradient_list)
...
#                         torch.save(gradient_dual, git_ignore_folder + 'grad_dual')
#                         
#                         torch.save(hessian_para_prod, git_ignore_folder + 'hessian_para_prod')
#                         
#                         print("here")
...
#             init_model(model, get_devectorized_parameters(updated_para, input_dim, hidden_dims, output_dim))
    ...
    return para, res_para_list, res_grad_list


def estimate_vectorized_full_grad(is_GPU, device, delta_u_s, delta_v, para_delta, perturbed_para_all, perturbed_para_all_inverse, prev_grad):
    

    
    
#     scales = torch.mm(para_delta.view(1,-1), perturbed_para_all_inverse)
    
    scales = torch.sum(para_delta.view(-1,1)*perturbed_para_all_inverse, 0)
    
#     print(scales - scales2)
    
    
#     print(scales)
    
#     res_grad_delta = torch.mm(torch.mm(scales.view(1,-1), delta_u_s), torch.t(delta_v))
    res_grad_delta = torch.mm(scales.view(1,-1), delta_u_s)
    
    res_grad = get_all_vectorized_parameters1(prev_grad).view(1,-1) + res_grad_delta.view(1,-1)
    
#     if is_GPU:
#         delta_u_s = delta_u_s.to('cpu')
# 
...
def model_update_provenance_test4(init_epochs, period, length, perturbed_para_all, perturbed_para_all_inverse, sub_v, delta_para_sub_V_list, delta_para_sub_U_list, max_epoch, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, gradient_list_all_epochs_tensor, para_list_all_epochs_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, cached_size, delta_ids, m, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device):
    ...
    grad_full_res = torch.zeros([total_shape_size], dtype = torch.double)
...
#                     alpha = learning_rate_all_epochs[i]
#                     
#                     exp_para = exp_para_list_all_epochs[i]
# 
# #                     print("iteration::", i)
# #                 
# #                     print("parameter diff::")
# #                 
# #                     compute_model_para_diff(exp_para, para)
#                 
# #                 print("gradient diff::")
# #                 
# #                     compute_model_para_diff(exp_gradients, get_devectorized_parameters(gradients, full_shape_list, shape_list))
# 
#                     para = get_devectorized_parameters((1-learning_rate*regularization_coeff)*curr_para - learning_rate*expect_gradients, full_shape_list, shape_list)
#                     
#                     recorded += 1
...
#             else:
            ...
            prev_para_list = para_list_GPU_tensor[cached_id]

            delta_para = get_all_vectorized_parameters1(para) - prev_para_list
            
            
            vec_para = get_all_vectorized_parameters1(para)
            
            
#             vec_para2 = get_all_vectorized_parameters0(para, full_shape_list, shape_list, grad_full_res)
            
#             print(torch.norm(vec_para - vec_para2))
            
            tt4 = time.time()
            overhead += (tt4 - tt3)
            delta_u_s = delta_para_sub_U_list[i]
            
            delta_v = delta_para_sub_V_list[i]
            
            tt1 = time.time()
            
            if is_GPU:
                delta_u_s = delta_u_s.to(device)
                
                delta_v = delta_v.to(device)
            
            delta_u_s = torch.mm(delta_u_s, torch.t(delta_v))
            
            
            
            
            scales = torch.sum(delta_para.view(-1,1)*perturbed_para_all_inverse, 0)
    
            res_grad_delta = torch.mm(scales.view(1,-1), delta_u_s)
            tt2 = time.time()
            
#             tt3 = time.time()
            gradient_full = grad_list_GPU_tensor[cached_id].view(1,-1)
            
            gradient_full  += res_grad_delta.view(1,-1)
...
#             overhead2 += (tt5 - tt4)
#             gradient_full = estimate_vectorized_full_grad(is_GPU, device, delta_u_s, delta_v, delta_para, perturbed_para_all, perturbed_para_all_inverse, grad_list_GPU_tensor[cached_id])
            
            
            overhead4 += (tt2  -tt1)
            if gradient_dual is not None:
                gradients = (gradient_full*(end_id-j) - get_all_vectorized_parameters1(gradient_dual)*curr_matched_ids_size)/(end_id-j - curr_matched_ids_size)
            else:
                gradients = gradient_full
            
            exp_gradients = exp_gradient_list_all_epochs[i]
            ...
            gradients += regularization_coeff*vec_para
            
            vec_para = vec_para - learning_rate*gradients
...
def model_update_provenance_test3_multi(num_epochs, init_model_para, exp_res_para_list, period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, gradient_list_all_epochs_tensor, para_list_all_epochs_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, cached_size, delta_ids, m, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device, end_cached_id):
...
#     random.seed(random_seed)
    ...
    for r in range(len(delta_ids)):
        ...
        delta_id = delta_ids[r]
        
        curr_delta_id = torch.tensor([delta_id])

        remaining_ids_list_all_epochs = []
...
#         cur_exp_para_list_all_epochs = exp_para_list_all_epochs[r]
#             
#         cur_exp_grad_list_all_epochs = exp_gradient_list_all_epochs[r]
        t11 = time.time()
        if r == 0:
...
#             for k in range(len(random_ids_multi_super_iterations)):
        ...
        #             print(i, len(random_ids_list))
                    ...
                    if curr_matched_id_num > 0:
                        random_ids_list.append(curr_matched_ids)
                        removed_batch_empty_list.append(False)
                        curr_remaining_ids = get_remaining_subset_data_per_epoch(random_ids[j:end_id], curr_matched_ids)
                    else:
                        random_ids_list.append(random_ids[0:1])
                        curr_remaining_ids = random_ids[j:end_id]
                        removed_batch_empty_list.append(True)
                    
                    remaining_ids_list.append(curr_remaining_ids)
                ...
                remaining_ids_list_all_epochs.append(remaining_ids_list)    
        
        
        else:
              
            init_model(model, init_model_para)
...
#                 for j in range(0, dim[0], batch_size):
                ...
                for j in range(len(prev_random_ids_list)):
                
                
        #             print(i, len(random_ids_list))
                    
                    end_id = jj + batch_size
                    ...
                    if curr_matched_id_num > 0:
                        random_ids_list.append(curr_matched_ids)
                        removed_batch_empty_list.append(False)
                        curr_remaining_ids = get_remaining_subset_data_per_epoch(prev_random_ids_list[j], curr_matched_ids)
                    else:
                        random_ids_list.append(random_ids[0:1])
                        removed_batch_empty_list.append(True)
                        curr_remaining_ids = prev_random_ids_list[j]
...
#             if r > 0:
            remaining_ids_list = remaining_ids_list_all_epochs[k]
    ...
    #         all_indexes = np.sort(sort_idx[delta_ids])
            ...
            curr_init_epochs = init_epochs
#             if k == 0:
#                 i = torch.nonzero(torch.tensor(removed_batch_empty_list).view(-1) == False)[0].item() - 1
#                 if i < 0:
#                     i = 0
#                  
#                 init_model(model, get_devectorized_parameters(para_list_all_epochs_tensor[i], full_shape_list, shape_list))
...
#                     cached_id = 0
#             
#                 print("curr_init_epochs::", k, curr_init_epochs, i, init_epochs)
...
#                         print(i)
                    ...
                    curr_matched_ids2 = remaining_ids_list[p]#random_ids[j:end_id]
...
#                         print(r, i)
#                             
#                         print("remaining size::", curr_matched_ids2.shape[0])
#                         
#                         print("delta size::", curr_matched_ids_size)
#                         if r > 5:
# #                              
# #                            
#                             print("iteration1::", i)
#                             print("para diff::")
#               
#                             compute_model_para_diff(para, cur_exp_para_list_all_epochs[i])
#                              
# #                             print(torch.norm(get_all_vectorized_parameters1(para) - para_list_all_epochs_tensor[i]))
#                                    
#                             print("gradient diff::") 
#                                   
#                             compute_model_para_diff(get_devectorized_parameters(expect_gradients, full_shape_list, shape_list), cur_exp_grad_list_all_epochs[i])
#                               
#                             print("here")
#                               
#                               
...
# #                             print("para diff 2::")
# #             
# # #                             compute_model_para_diff(para, para_list_all_epochs[i])
# #                               
# #                             print(torch.norm(curr_para - para_list_GPU_tensor[cached_id]))
# #                                  
# #                             print("gradient diff 2::") 
# #                               
# #                             print(torch.norm(gradient_full - grad_list_GPU_tensor[cached_id]))
#                                 
# #                             compute_model_para_diff(get_devectorized_parameters(expect_gradients, full_shape_list, shape_list), cur_exp_grad_list_all_epochs[i])
#                               
#                               
...
#                             print("here")
    ...
    #                     res_grad_list.append(expect_gradients + regularization_coeff*curr_para)
                        ...
                        grad_list_GPU_tensor[cached_id] = expect_gradients
                        

                        
#                         para_list_all_epochs_tensor[(batch_id - 1)*cached_size + cached_id] = curr_para.cpu()
#                         
#                         gradient_list_all_epochs_tensor[(batch_id - 1)*cached_size + cached_id] = expect_gradients.cpu()
...
#                             hessian_para_prod, zero_mat_dim, curr_Y_k, curr_S_k, sigma_k, mat_prime = cal_approx_hessian_vec_prod0_3(S_k_list, Y_k_list, i, init_epochs, m, vec_para_diff, period, is_GPU, device)
    ...
    #                         hessian_para_prod = cal_approx_hessian_vec_prod0(S_k_list, Y_k_list, i, init_epochs, m, vec_para_diff, period)
        ...
        #                 cal_approx_hessian_vec_prod0(S_k_list, Y_k_list, i, init_epochs, m, v_vec.view(-1,1), period)
            ...
            #             para_diff = (get_all_vectorized_parameters(para) - get_all_vectorized_parameters(para_list_all_epochs[i]))
                        ...
                        if gradient_dual is not None:
                            is_positive, final_gradient_list, exp_gradient_full = compute_grad_final4(get_all_vectorized_parameters1(para), torch.t(hessian_para_prod), get_all_vectorized_parameters1(gradient_dual), grad_list_GPU_tensor[cached_id], para_list_GPU_tensor[cached_id], remaining_ids_list[p].shape[0], curr_matched_ids_size, learning_rate, regularization_coeff, is_GPU, device)
                            
                        else:
                            is_positive, final_gradient_list, exp_gradient_full = compute_grad_final4(get_all_vectorized_parameters1(para), torch.t(hessian_para_prod), None, grad_list_GPU_tensor[cached_id], para_list_GPU_tensor[cached_id], remaining_ids_list[p].shape[0], curr_matched_ids_size, learning_rate, regularization_coeff, is_GPU, device)
                    ...
                    if not is_positive:
                        ...
                        print("going back to explicit computation!!")
    ...
    #                         overhead2 += (t4  -t3)
        ...
        #                         Y_k_list.append((expect_gradients - get_all_vectorized_parameters1(gradient_list_all_epochs[i]).to(device)).view(-1))
                                ...
                                Y_k_list.append(gradient_full - grad_list_GPU_tensor[cached_id] + regularization_coeff*curr_s_list)
...
#                         para_list_all_epochs_tensor[(batch_id - 1)*cached_size + cached_id] = get_all_vectorized_parameters1(para).cpu()
#                         
#                         gradient_list_all_epochs_tensor[(batch_id - 1)*cached_size + cached_id] = exp_gradient_full.cpu()
#                         t11 = time.time()
                        para_list_GPU_tensor[cached_id] = get_all_vectorized_parameters1(para)
                        
                        grad_list_GPU_tensor[cached_id] = exp_gradient_full
...
#                         if r > 2:
#                             
#                             print(i, r, curr_matched_ids_size)
#                         if r > 5:
#                              
#                             if r > 0 and i == 133:
# #                                 print(curr_random_ids.shape[0])
#                                  
#                                 curr_random_ids = torch.load(git_ignore_folder + "tmp_curr_random_ids1")
#                                  
#                                 print(curr_random_ids.shape[0])                                
#                                  
#                             if r > 0 and i == 134:
# #                                 print(curr_random_ids.shape[0])
#                                  
#                                 curr_random_ids = torch.load(git_ignore_folder + "tmp_curr_random_ids2")
#                                 print(curr_random_ids.shape[0])                                
#                              
# #                             if not removed_batch_empty_list[i]:
#                             print("iteration2::", i, curr_matched_ids2.shape[0], curr_matched_ids_size)
# #                             else:
# #                                 print("iteration2::", i, curr_matched_ids2.shape[0])
#                              
#                             print("para diff::")
#               
#                             compute_model_para_diff(para, cur_exp_para_list_all_epochs[i])
...
#                             print("gradient diff::") 
#                                  
#                             exp_grad_list = get_all_vectorized_parameters1(cur_exp_grad_list_all_epochs[i]) + regularization_coeff*get_all_vectorized_parameters1(cur_exp_para_list_all_epochs[i])
#                               
#                             compute_model_para_diff(get_devectorized_parameters(final_gradient_list, full_shape_list, shape_list), get_devectorized_parameters(exp_grad_list, full_shape_list, shape_list))
#                              
#                             print("here")
...
# #                             compute_model_para_diff(para, para_list_all_epochs[i])
#                               
#                             print(torch.norm(get_all_vectorized_parameters1(para) - para_list_GPU_tensor[cached_id]))
#                                  
#                             print("gradient diff 2::") 
#                               
#                             print(torch.norm(exp_gradient_full - grad_list_GPU_tensor[cached_id]))
#                           
#                           
#                         print("here")
    ...
    #                 overhead += (t2 - t1)
                ...
                if cached_id%cached_size == 0:
                    ...
                    para_list_all_epochs_tensor[(batch_id - 1)*cached_size:batch_id*cached_size].copy_(para_list_GPU_tensor[0:cached_size])
                    
                    gradient_list_all_epochs_tensor[(batch_id - 1)*cached_size:batch_id*cached_size].copy_(grad_list_GPU_tensor[0:cached_size])
...
#         print(torch.norm(exp_res_para_list[r] - get_all_vectorized_parameters1(para)))
        
        
        
        
        
        prev_random_ids_list_all_epochs =  remaining_ids_list_all_epochs
        
        end_cached_id = cached_size
    
        if end_cached_id > para_list_all_epochs_tensor.shape[0]:
            end_cached_id =  para_list_all_epochs_tensor.shape[0]
        
        
        para_list_all_epochs_tensor[(batch_id - 1)*cached_size:(batch_id - 1)*cached_size + end_cached_id].copy_(para_list_GPU_tensor[0:end_cached_id])
                    
        gradient_list_all_epochs_tensor[(batch_id - 1)*cached_size: (batch_id - 1)*cached_size + end_cached_id].copy_(grad_list_GPU_tensor[0:end_cached_id])
    
        
        para_list_GPU_tensor[0:end_cached_id].copy_(para_list_all_epochs_tensor[0:end_cached_id])
    
        grad_list_GPU_tensor[0:end_cached_id].copy_(gradient_list_all_epochs_tensor[0:end_cached_id])
...
'''period, 1, init_epochs, None, None, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, grad_list_all_epoch_tensor, para_list_all_epoch_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, cached_size, 2, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device'''

def model_update_provenance_test3_jackknife(max_epoch, period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, gradient_list_all_epochs_tensor, para_list_all_epochs_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, para_list_all_epochs, gradient_list_all_epochs, cached_size, m, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device):
...
#                         if r > 0:
#     
#                             print("compute para diff::")
#           
#                             compute_model_para_diff(list(exp_para_list_all_epochs[i]), para)
#                           
#                             print("compute grad diff::")
#                           
#                             compute_model_para_diff(list(exp_gradient_list_all_epochs[i]), get_devectorized_parameters(expect_gradients, full_shape_list, shape_list))
...
#                 id_start = id_end
        ...
        if summed_delta_para is None:
            summed_delta_para = get_all_vectorized_parameters(para)
        else:
            summed_delta_para += get_all_vectorized_parameters(para)
    ...
    return summed_delta_para, para, res_para_list, res_grad_list




def model_update_provenance_test1(period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, gradient_list_all_epochs, para_list_all_epochs, epoch, delta_ids, m, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device):
...
#                 t7 = time.time()
                ...
                if is_GPU:
                    expect_gradients = expect_gradients.cpu()
...
def model_update_provenance_test1_logs(period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, gradient_list_all_epochs, para_list_all_epochs, epoch, delta_ids, m, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device):
...
#     count3 = 0
    ...
    derived_para_list_all_epochs = []
    
    derived_gradient_list_all_epochs = []
    
    all_explicit_cal_iterations = []
...
#                 _,next_items = enum_loader.__next__()
#                 print(i)
                all_explicit_cal_iterations.append(i)
...
#             derived_para_list_all_epochs.append(para.copy())
            init_model(model, para)

            append_gradient_list(derived_gradient_list_all_epochs, None, derived_para_list_all_epochs, model, None, is_GPU, device)
...
#     print("count3::", count3)
    ...
    return para, derived_para_list_all_epochs, derived_gradient_list_all_epochs, all_explicit_cal_iterations
...
def model_update_provenance_test1_4(max_epoch, period, length, init_epochs, res_para_list, res_grad_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, dataset_train, model, grad_list_all_epoch_tensor, para_list_all_epoch_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, cached_size, delta_ids, m, learning_rate_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device):
...
#                             S_k_list.popleft()
                    ...
                    if k>0 or (k == 0 and p > 0):
                        ...
                        curr_Y_k = gradient_full - grad_list_GPU_tensor[cached_id] + regularization_coeff*curr_S_k
...
#                         dot_res = torch.mm(curr_S_k, torch.t(curr_Y_k))
                        dot_res = torch.dot(curr_S_k.view(-1), curr_Y_k.view(-1))
...
#                     exp_gradient = res_grad_list[i]
# #               
#    
#                        
#                     exp_param = res_para_list[i]
...
#                     print("gradient diff::")
#                     
#                     print(torch.norm(exp_gradient - (expect_gradients + regularization_coeff*curr_para)))
...
#                     print("grad diff::")
#                     
#                     print(torch.norm(exp_gradient - final_gradient_list))
    ...
    #                 else:
                    ...
                    with torch.no_grad():
                        ...
                        if k>0 or (k == 0 and p > 0):
                            ...
                            curr_S_k = (get_all_vectorized_parameters1(para) - para_list_GPU_tensor[cached_id])
...
#                             print("here!!")
                        ...
                        para = get_devectorized_parameters((1-learning_rate*regularization_coeff)*get_all_vectorized_parameters1(para) - learning_rate*expect_gradients, full_shape_list, shape_list)
                else:
#                     update_para_final(para, final_gradient_list, learning_rate, regularization_coeff, exp_gradient, exp_param)
                    vec_para = update_para_final2(para, final_gradient_list, learning_rate, regularization_coeff, None, None)
...
#                     if (i-last_explicit_training_iteration)/period >= 1:
                    if (i-last_explicit_training_iteration) % period == 1:
                        ...
                        zero_mat_dim, curr_Y_k, curr_S_k, sigma_k, mat_prime = prepare_hessian_vec_prod0_4(S_k_list, Y_k_list, i, init_epochs, m, is_GPU,device)
...
def get_sorted_random_ids(random_ids_multi_super_iterations):
    ...
    return sorted_ids_multi_super_iterations
                
        
def save_random_id_orders(git_ignore_folder, random_ids_multi_super_iterations):
    
    
    sorted_ids_multi_super_iterations = get_sorted_random_ids(random_ids_multi_super_iterations)
...
def get_train_test_data_loader_by_name_lr(data_preparer, Model, name, specified_batch_size):
    ...
    dataset_train, dataset_test, data_train_loader, data_test_loader = get_train_test_data_loader_lr(Model, train_X, train_Y, test_X, test_Y, specified_batch_size)
    
    return dataset_train, dataset_test, data_train_loader, data_test_loader



def get_train_test_data_loader_by_name_lr_transfer(data_preparer, transfer_model, transfer_model_name, Model, name, specified_batch_size, is_GPU, device):
    
    
    function=getattr(Data_preparer, "prepare_" + name)
    
    train_X, train_Y, test_X, test_Y = function(data_preparer, transfer_model, transfer_model_name, is_GPU, device)
