...
#     curr_X_Y_mult = X_Y_mult[delta_data_ids]
    ...
    first_derivative = compute_first_derivative(update_X, update_Y, dim, model_origin, num_class)
...
#     compute_first_derivative_single_data2(torch.index_select(X_Y_mult, 0, delta_data_ids), Variable(model_origin), dim)
    ...
    delta_theta = torch.t((torch.mm(Hessian_inverse, first_derivative)/dim[0]).view(num_class, dim[1]))
    ...
    updated_theta = model_origin - delta_theta
...
#     print('gap::', compute_first_derivative_single_data(X_Y_mult, selected_rows, expected_theta))
    ...
    print('angle::', torch.dot(torch.reshape(expected_theta, [-1]), torch.reshape(updated_theta, [-1]))/(torch.norm(torch.reshape(expected_theta, [-1]))*torch.norm(torch.reshape(updated_theta, [-1]))))
