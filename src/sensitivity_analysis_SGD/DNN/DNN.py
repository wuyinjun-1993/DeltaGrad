...
def model_update_standard_lib(num_epochs, X, Y, error, model, beta, random_ids_multi_super_iterations, selected_rows, batch_size, learning_rate_all_epochs):
    count = 0
#     for epoch in range(num_epochs):
    loss = np.infty
...
def model_update_provenance_test1(period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, X, Y, model, gradient_list_all_epochs, para_list_all_epochs, epoch, delta_ids, input_dim, hidden_dims, output_dim, m, learning_rate_all_epochs, beta, selected_rows, error, delta_X,delta_Y, update_X, update_Y, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim):
...
#                         print("secand condition check::", torch.dot(S_k_list[-1], Y_k_list[-1]))
    ...
    #                 
    #                 expect_gradient_full = get_all_vectorized_parameters(model.get_all_gradient())
...
#                     print("grad_diff::")
#                       
#                     compute_model_para_diff(get_devectorized_parameters(expect_gradients, input_dim, hidden_dims, output_dim), exp_gradient)
...
#                             mat_2_inverse = torch.inverse(mat_2)
                            ...
                            mat_prime_inverse = torch.inverse(mat_prime)
                            
                            
#                             torch.mm(mat_1, torch.mm(mat_2
#                             mat = torch.mm(mat_1_inverse, mat_2_inverse)
#                             print(torch.norm(mat - mat_prime_inverse))
                        
                        hessian_para_prod = compute_approx_hessian_vector_prod_with_prepared_terms(zero_mat_dim, curr_Y_k, curr_S_k, sigma_k, mat_prime_inverse, vec_para_diff)
    #                     hessian_para_prod = compute_approx_hessian_vector_prod_with_prepared_terms2(zero_mat_dim, curr_Y_k, curr_S_k, sigma_k, mat_1, mat_2, v_vec2, shape_list)
                        ...
                        hessian_para_prod2 = cal_approx_hessian_vec_prod0_2(S_k_list, Y_k_list, i, init_epochs, m, vec_para_diff, period)
                        
                        hessian_para_prod = cal_approx_hessian_vec_prod0(S_k_list, Y_k_list, i, init_epochs, m, vec_para_diff, period)
                    
                        print(torch.norm(hessian_para_prod - hessian_para_prod2))
        #             print('para_diff::', torch.norm(v_vec))
        #             
        #             print('para_angle::', torch.dot(get_all_vectorized_parameters(para).view(-1), get_all_vectorized_parameters(para_list_all_epochs[i]).view(-1))/(torch.norm(get_all_vectorized_parameters(para).view(-1))*torch.norm(get_all_vectorized_parameters(para_list_all_epochs[i]).view(-1))))
...
def model_update_provenance_test1_varied_period(max_period, length, init_epochs, truncted_s, extended_Y_k_list, exp_gradient_list_all_epochs, exp_para_list_all_epochs, X, Y, model, gradient_list_all_epochs, para_list_all_epochs, epoch, delta_ids, input_dim, hidden_dims, output_dim, m, learning_rate_all_epochs, beta, selected_rows, error, delta_X,delta_Y, update_X, update_Y, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim):
...
#         num_epochs = int((batch_size/default_batch_size)*default_epoch_num)
    ...
    hidden_dim = [50, 10] #hidden layer dim is one of the hyper parameter and it should be chosen and tuned. For now I only say 150 there is no reason.
...
#         
#         exp_gradient_list = model.get_all_gradient()
#         
#         
#         
...
#         
#         expected_gradient = model.get_all_gradient()#(list(model.parameters()))
#         
#         
...
#         
#         para_list3 = model.get_all_parameters
#         
...
#         
#         expected_gradient_gap = get_all_vectorized_parameters(model.get_all_gradient()) - get_all_vectorized_parameters(gradient_list2)/X.shape[0]
#         
...
#         compute_derivative_one_more_step(model, error, X, Y)
#         
#         expected_full_gradient_list2 = model.get_all_gradient()
#             
