...
#     model, count = model_update_standard_lib_stochastic(batch_size, max_epoch, update_X, update_Y, alpha, error, model)
    ...
    print('delta_data_id_length::', delta_data_ids.shape[0])
    
    
    
    
    while id < delta_data_ids.shape[0]:
        
        print('delta_data_id::', id)
        
        curr_delta_data_ids = delta_data_ids[id:id+id_sublist_count]
        ...
        init_model_list.append(get_model_para(model))

        t1 = time.time()
        model, count, exp_para_list_all_epochs, exp_gradient_list_all_epochs, _ = model_update_standard_lib(max_epoch, dataset_train, model, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, curr_delta_data_ids, batch_size, learning_rate_all_epochs, criterion, optimizer, para_list_all_epochs, gradient_list_all_epochs, is_GPU, device, record_params)
    
        t2 = time.time()
        
        final_model_list.append(get_model_para(model))
        
        id_list.append(id)
        
        torch.save(exp_gradient_list_all_epochs, git_ignore_folder + 'expected_gradient_list_all_epochs_' + str(id))
      
        torch.save(exp_para_list_all_epochs, git_ignore_folder + 'expected_para_list_all_epochs_' + str(id))

        id += id_sublist_count
    
    torch.save(torch.tensor(id_list), git_ignore_folder + 'delta_data_id_index_list')
