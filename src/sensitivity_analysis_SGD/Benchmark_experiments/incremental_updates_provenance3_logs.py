...
def n_choose_k_from_list(all_explicit_cal_iterations, k, i):
    all_result_ids = []
    
    if k == len(all_explicit_cal_iterations) - i:
        all_result_ids.append(all_explicit_cal_iterations[i:])
        return all_result_ids
    
    
    
    all_res1 = n_choose_k_from_list(all_explicit_cal_iterations, k, i+1)
    
    
    all_res2 =  n_choose_k_from_list(all_explicit_cal_iterations, k - 1, i+1)
    
    for p in range(len(all_res2)):
        
        all_res2[p].append(i)
        
    all_result_ids.extend(all_res1)
    
    all_result_ids.extend(all_res2)
    
    return all_result_ids
        
    
def n_choose_2_from_list(all_explicit_cal_iterations):
    
    all_ids_comb_list = []
    
    
    for i in range(len(all_explicit_cal_iterations)):
        
        for j in range(len(all_explicit_cal_iterations)):
            
            if i < j:
                all_ids_comb_list.append([all_explicit_cal_iterations[i], all_explicit_cal_iterations[j]])
    
    
    return all_ids_comb_list
    

def compute_s_j_1_j_m(para_list_all_epochs, derived_para_list_all_epochs, all_explicit_cal_iterations):
    
    all_ids_comb_list = n_choose_2_from_list(all_explicit_cal_iterations)
    
    
    for i in range(len(all_ids_comb_list)):
        id1 = all_ids_comb_list[i][0]
        
        id2 = all_ids_comb_list[i][1]
        
        para_diff1 = get_all_vectorized_parameters(para_list_all_epochs[id1]) - get_all_vectorized_parameters(derived_para_list_all_epochs[id1])
        
        para_diff2 = get_all_vectorized_parameters(para_list_all_epochs[id2]) - get_all_vectorized_parameters(derived_para_list_all_epochs[id2])
        ...
        for j in range(id1, id2):
            ...
            curr_para_norm_diff = torch.norm(get_all_vectorized_parameters(para_list_all_epochs[j]) - get_all_vectorized_parameters(derived_para_list_all_epochs[j])) 
            
            if curr_para_norm_diff > max_norm:
                max_norm = curr_para_norm_diff
            
        
        para_mat = torch.cat([para_diff1, para_diff2], 0)
        
        para_mat = para_mat/max_norm
        
        
        u,s,v = np.linalg.svd(para_mat.numpy())
...
#     model_para_list = model_update_provenance_test1_advanced(1, 20, 1, init_epochs, None, None, exp_gradient_list_all_epochs, exp_para_list_all_epochs, origin_X, origin_Y, model, gradient_list_all_epochs, para_list_all_epochs, max_epoch, delta_data_ids, input_dim, hidden_dims, output_dim, 3, alpha, beta, selected_rows, error, delta_X, delta_Y, update_X, update_Y, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, batch_size, dim)
    ...
    compute_s_j_1_j_m(para_list_all_epochs, derived_para_list_all_epochs, all_explicit_cal_iterations)
