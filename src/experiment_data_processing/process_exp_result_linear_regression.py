...
def write_to_csv_file(file_name, results, output_file_suffix, need_relative_errors, need_batches):
    ...
    if not need_batches:
        ...
        relative_arr = np.zeros((noise_rate_num, 6))
...
#     approach_list.insert(0, '')
    ...
    if not need_batches:
        ...
        if need_relative_errors:
            ...
            convert2df(relative_arr, noise_rate_list, [provenance_label, provenance_opt_label, influence_label, iteration_batch_label_1, iteration_batch_label_2, iteration_batch_label_3]).to_excel(writer, sheet_name='relative_err', index=False)
...
file_name = '../../scripts/output_linear_regression_less_features'
...
if __name__ == '__main__':
    ...
    with open(file_name + '.txt') as fp:  
        ...
        labels = [origin_label, standard_lib_label, iteration_label, closed_form_label, linview_label, provenance_label, provenance_opt_label]
          
        start_prefixes = [start_standard_lib_label, start_iteration_label, start_closed_form, start_linview, start_provenance_label, start_provenance_opt_label, start_influence_function]
          
          
        training_time_labels = [training_time_label, training_time_standard_lib_label, training_time_iteration_label, training_time_closed_form_label, training_time_linview_label, training_time_provenance_label, training_time_provenance_label]      
        
        expected_state = 2
        
    #     start_output = False
        last_label = influence_label
        
        while line:
            
            line = fp.readline()
            
            if line.startswith(start_provenance_label):
                y = 1
                y+= 1
                
#             if line.startswith(start_provenance_opt_label):
#                 print(line)
#                 
#             if line.startswith(start_influence_function):
#                 print(line)
    ...
    #             print(line)
            ...
            if line.startswith('change_data_values'):
                state = 1
                continue
...
#                     continue
    ...
    #             curr_results[str(noise_rate)] = {training_time_label: avg_training_time, training_accuracy_label: avg_training_accuracy, test_accuracy_label: avg_test_accuracy, absolute_error_label: avg_absolute_error, relative_error_label: avg_relative_error}
                ...
                if not is_change_data_value and not is_random_flipping and not is_epoch_num_test and not is_cut_off_thres:
                    ...
                    if last_label not in add_noise_results:
                        add_noise_results[last_label] = curr_results
                        
                    else:
                        add_noise_results[last_label][str(noise_rate)] = curr_results[str(noise_rate)]
                        
                else:
                    
                    if is_change_data_value:
                    
                        if last_label not in change_data_value_results:
                            change_data_value_results[last_label] = curr_results
                            
                        else:
                            change_data_value_results[last_label][str(noise_rate)] = curr_results[str(noise_rate)]
                            
                    else:
                        
                        if is_random_flipping:
                            if last_label not in random_results:
                                random_results[last_label] = curr_results
                                
                            else:
                                random_results[last_label][str(noise_rate)] = curr_results[str(noise_rate)]
                                
                        else:
                            
                            if is_epoch_num_test:
                                if last_label not in epoch_num_results:
                                    epoch_num_results[last_label] = curr_results
                                    
                                else:
                                    epoch_num_results[last_label][str(noise_rate)] = curr_results[str(noise_rate)]
                                    
                            else:
                                
                                if is_cut_off_thres:
                                    if last_label not in cut_off_thres_results:
                                        cut_off_thres_results[last_label] = curr_results
                                        
                                    else:
                                        cut_off_thres_results[last_label][str(noise_rate)] = curr_results[str(noise_rate)]
...
#             curr_results[str(noise_rate)] = {training_time_label: avg_training_time, training_accuracy_label: avg_training_accuracy, test_accuracy_label: avg_test_accuracy, absolute_error_label: avg_absolute_error, relative_error_label: avg_relative_error}
        ...
        write_to_csv_file(file_name, add_noise_results, 'change_data_value', need_relative_errors, need_batches)
