...
def compute_model_parameter_by_iteration3(batch_size, exp_gradient_list_all_epochs, exp_para_list_all_epochs, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, dim, theta,  X, Y, selected_rows, num_class, max_epoch, learning_rate_all_epochs, beta):
...
#             
#             exp_learning_rate = torch.load(git_ignore_folder + 'tmp_learning_rate')
#             
#             exp_curr_random_ids = torch.load(git_ignore_folder + 'tmp_rand_ids')
#             
#             tmp_X = torch.load(git_ignore_folder + 'tmp_X')
#             
#             print(torch.norm(exp_batch_X - batch_X))
#                   
#             print(torch.min(torch.tensor(exp_curr_random_ids) - curr_matched_ids))
#             
#             print(torch.max(torch.tensor(exp_curr_random_ids) - curr_matched_ids))
#             
#             print(torch.norm(tmp_X - X))
...
#             print('theta_diff:', torch.norm(theta - list(exp_para_list_all_epochs[epoch])[0].T))
            
            reshape_theta -= learning_rate_all_epochs[epoch] * output
...
#             list(origin_model.parameters())[0].T)
#             print('grad_diff:', torch.norm(grad.view(theta.shape) - list(exp_gradient_list_all_epochs[epoch])[0]))
...
#     print('t_time2::', t_time2)
    ...
    return theta, grad_list, theta_list
...
#                 t3 = time.time()
            ...
            sub_term2 = term2[epoch].view(-1,1) - sub_term2
...
# 
#             sub_term2_list.append(sub_term2.clone())
#             
#             x_sum_by_list.append(delta_x_sum_by_class.clone())
#             
#             sub_term_1_theta_list.append(vectorized_sub_term_1)
...
#             theta_list.append(vectorized_theta.clone())
            
            theta = vectorized_theta.view(num_class, dim[1]).T
            
#             if epoch < len(theta_list):
#                 print("theta_diff::", torch.norm(theta_list[num] - theta))
...
#                 
#                 print(torch.norm(vectorized_theta - exp_theta))
#                  
#                 print(torch.norm(output - grad_list[epoch]))
#             else:
...
def compute_model_parameter_by_approx_incremental_1_2(cut_off_epoch, output_list, exp_x_sum_by_class_list, theta_list, grad_list, origin_X, origin_Y, weights, offsets, delta_ids, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, term1, term2, dim, theta, max_epoch, learning_rate_all_epochs, beta, batch_size, num_class, x_sum_by_class_list, avg_term2, u_list, v_s_list):
...
#                 full_term2 = avg_term2    
            ...
            if curr_matched_ids_size > 0:
                ...
                torch.save(delta_x_sum_by_class, git_ignore_folder + "tmp_delta_x_sum_by_class")
                
                torch.save(sub_term2_cp, git_ignore_folder + "tmp_sub_term2")
...
#                 if i == 0:
#                  
#                     origin_batch_X = torch.load(git_ignore_folder + 'tmp_batch_X')
#                     
#                     origin_batch_term1 = torch.load(git_ignore_folder + 'tmp_batch_term1')
#                     
#                     origin_batch_weights = torch.load(git_ignore_folder + 'tmp_batch_weights')
#                     
#                     origin_batch_offsets = torch.load(git_ignore_folder + 'tmp_batch_offsets')
#                     
#                     origin_curr_rand_ids = torch.load(git_ignore_folder + 'tmp_rand_ids')
#                      
#                     exp_sub_u = torch.load(git_ignore_folder + 'tmp_sub_u')
#                     
#                     exp_sub_v = torch.load(git_ignore_folder + 'tmp_sub_v')
#                      
#                     full_batch_X = X[random_ids[i:end_id]]            
#     
#                     print(torch.norm(full_batch_X - origin_batch_X))
#                     
#                     
#                   
#                     coeff_rand_ids = random_ids[i:end_id] + k*dim[0]
#                           
#                     full_batch_weights = weights[coeff_rand_ids]
#                     full_batch_offsets = offsets[coeff_rand_ids]
#                      
#                     batch_term1 = prepare_sub_term_1(full_batch_X, full_batch_weights.view(batch_size, num_class*num_class), full_batch_X.shape, num_class)
#          
#                     sub_u, sub_v = compute_single_svd(epoch, batch_term1, batch_size)
#                      
#                     es_batch_term1 = torch.mm(u_list[epoch], v_s_list[epoch])
#          
#                     print(torch.norm(torch.tensor(sub_u) - u_list[epoch]))
#                      
#                     print(torch.norm(torch.tensor(sub_v) - v_s_list[epoch]))
         
#                     print(torch.norm(sub_u - exp_sub_u))
#                     
#                     print(torch.norm(sub_v - exp_sub_v))
#                 batch_term2 = prepare_sub_term_2(full_batch_X, full_batch_offsets, full_batch_X.shape, num_class)
                     
#                 exp_sub_batch_term2 = prepare_sub_term_2(batch_delta_X, batch_offsets, batch_delta_X.shape, num_class)
#                  
#                 exp_sub_batch_term1 = prepare_sub_term_1(batch_delta_X, batch_weights.view(curr_matched_ids_size, num_class*num_class), batch_delta_X.shape, num_class)
#              
#                 print(torch.norm(sub_term2_cp.view(-1) - exp_sub_batch_term2.view(-1)))
                 
#                 print(torch.norm(exp_sub_batch_term1.view(-1,1) - vectorized_sub_term_1))
                 
                print("here")
#             exp_batch_X = torch.load(git_ignore_folder + 'tmp_batch_X')
#             
#             exp_batch_weights = torch.load(git_ignore_folder + 'tmp_batch_weights')
#             exp_sub_v = torch.load(git_ignore_folder + 'tmp_sub_v')
#             
...
#             theta_list.append(vectorized_theta.clone())
            ...
            print(torch.norm(theta_list[epoch] - theta))
...
def compute_model_parameter_by_approx_incremental_4_4(delta_X, s, M, M_inverse, theta_list, origin_X, origin_Y, weights, offsets, delta_ids, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, term1, term2, dim, theta, max_epoch, cut_off_epoch, learning_rate_all_epochs, beta, batch_size, num_class, x_sum_by_class_list, avg_term2, u_list, v_s_list):
...
#                     sub_term2 = (prepare_sub_term_2(batch_delta_X, batch_offsets, batch_delta_X.shape, num_class)).view(-1,1)
                    ...
                    sub_term2 = torch.mm(torch.t(batch_offsets.view(curr_matched_ids_size, num_class)), batch_delta_X).view(-1,1)
...
#                     curr_s = s_list[epoch - (cut_off_epoch - min_batch_num_per_epoch)]
                    ...
                    all_delta_sub_term_2 += (term2[epoch].view(-1,1) - sub_term2)
...
#                     output = torch.mm(term1[epoch], vectorized_theta) - vectorized_sub_term_1 + (full_term2.view(-1,1) - sub_term2)
                ...
                output /= (end_id - i - curr_matched_ids.shape[0])
                
                output += beta*vectorized_theta  

#                 if epoch >= cut_off_epoch - 2:
# 
#                     print('output_diff::', torch.norm(output_list[epoch] - output))
#                     
#                     print('sub_term2_diff::', torch.norm(sub_term2_list[epoch] - (term2[epoch].view(-1,1) - sub_term2)))
#                     
#                     print('x_sum_by_class_diff:', torch.norm(x_sum_by_list[epoch] - x_sum_by_class))
#                     
#                     print('sub_term1_diff::', torch.norm(sub_term_1_theta_list[epoch] - vectorized_sub_term_1))
            
#                 , output_list, sub_term2_list, x_sum_by_list, sub_term_1_theta_list
...
#                 del output
                ...
                vectorized_theta -= learning_rate_all_epochs[epoch]*output
                
#                 theta_list.append(vectorized_theta.clone())
#                 
#                 grad_list.append(gradient.clone())
...
#                 exp_theta = theta_list[epoch]
#                 if epoch >= cut_off_epoch - 2:
#                     print('theta_diff::', torch.norm(vectorized_theta - exp_theta))
...
def compute_model_parameter_by_approx_incremental_4_5(delta_X, s, M, M_inverse, theta_list, origin_X, origin_Y, weights, offsets, delta_ids, random_ids_multi_super_iterations, sorted_ids_multi_super_iterations, term1, term2, dim, theta, max_epoch, cut_off_epoch, alpha, beta, batch_size, num_class, x_sum_by_class_list, avg_term2, u_list, v_s_list):
...
#     batch_num_per_epoch = int((X.shape[0] - 1)/batch_size) + 1
    ...
    Y = origin_Y
    delta_id_set = set(delta_ids.view(-1).tolist())
    vectorized_theta = Variable(torch.reshape(torch.t(theta), [-1,1]))
...
#         offsets_this_super_iteration = offsets_this_super_iteration[delta_ids]
        ...
        for i in range(0, X.shape[0], batch_size):
            ...
            curr_matched_ids = (get_subset_data_per_epoch(curr_rand_ids, delta_id_set))
...
def compute_x_sum_by_class_by_batch(X, Y, batch_size, num_class, random_ids_multi_super_iterations):
...
def eigen_decomposition(git_ignore_folder, term1, dim, num_class, batch_size, X, weights, cut_off_super_iteration):
...
#     if batch_size < term1.shape[1]:
    upper_bound = int(batch_size/svd_ratio + 0.5)
#     else:
#         upper_bound = int(term1.shape[1]/svd_ratio + 0.5)
...
#             non_zero_ids = np.array([0,1])
    ...
    difference = np.linalg.norm(res - curr_term1)
...
#         difference = np.linalg.norm(res - curr_term1)
    ...
    print(i, upper_bound, difference, np.linalg.norm(res - curr_term1)/np.linalg.norm(curr_term1))
    
    
    
    return sub_u*sub_s, sub_v
...
def eigen_decomposition3(git_ignore_folder, avg_term1):
...
#     v_list = []
    ...
    for k in range(curr_rand_ids_multi_super_iterations.shape[0]):
        ...
        for i in range(0, X.shape[0], batch_size):
            ...
            torch.save(torch.tensor(sub_v), git_ignore_folder + 'tmp_sub_v')
...
def capture_provenance(X, Y, dim, epoch, num_class, batch_size, mini_epochs_per_super_iteration, random_ids_multi_super_iterations, X_theta_prod_softmax_seq, X_theta_prod_seq):
...
#     while count < num:
        ...
        curr_coeff = curr_coeff[curr_coeff != np.inf]
        '''higgs::-5, cov_small: -1, heartbeat: 0.1, cifar10: 2'''
        curr_coeff = 0.01*torch.sum(curr_coeff[curr_coeff == curr_coeff])
        
#         if predict_labels[indices[i]] == Y[indices[i]]:
             
#         print(curr_coeff) 
            
    #         curr_coeff = torch.sum(curr_coeff[curr_coeff != np.inf])
            
        X[indices[i]] *= curr_coeff#torch.rand(X.shape[1], dtype = torch.double)
...
#     global alpha, beta, threshold
    ...
    theta_record_threshold = float(sys_args[11])
...
#         x_sum_by_class = compute_x_sum_by_class(X, Y, num_class, dim)
        ...
        capture_provenance(X, Y, dim, epoch, num_class, batch_size, mini_epochs_per_super_iteration, random_ids_multi_super_iterations_tensors, X_theta_prod_softmax_seq, X_theta_prod_seq)
        
        if dim[1]*num_class < max_para_num_opt:
    ...
    #     torch.save(X.mul(Y.type(torch.DoubleTensor)), git_ignore_folder + 'X_Y_mult')
        ...
        print('train_accuracy::', compute_accuracy2(X, Y.type(torch.DoubleTensor), res2))
