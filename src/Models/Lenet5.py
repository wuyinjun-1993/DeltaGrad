...
#             self.data = train_X.type(torch.DoubleTensor).view(train_X.shape[0], 1, train_X.shape[1], train_X.shape[2])
            
            self.data = train_X
            
            self.labels = train_Y.type(torch.DoubleTensor)
...
#             self.transformed_X = self.transformed_X.transpose(0,1).transpose(1,2)
        ...
        def transform_data(self, ids):
            ...
            return self.transformed_X[ids].view(ids.shape[0], 1, self.data.data.shape[1], self.data.data.shape[2]).type(torch.DoubleTensor), self.data.targets[ids]
...
#     [X, Y, test_X, test_Y] = load_data_multi_classes(True, file_name)
    ...
    data_train = LeNet5.MyDataset(torchvision.datasets.CIFAR10(git_ignore_folder + '/cifar10',
                   download=True,
                   transform=transforms.Compose([
                       transforms.Resize((32, 32)),
                       transforms.ToTensor()])))
    data_test = LeNet5.MyDataset(torchvision.datasets.CIFAR10(git_ignore_folder + '/cifar10',
                      train=False,
                      download=True,
                      transform=transforms.Compose([
                          transforms.Resize((32, 32)),
