...
#     gradient_list = torch.load(git_ignore_folder + 'gradient_list')
    ...
    added_random_ids_multi_super_iteration = get_sampling_each_iteration(random_ids_multi_super_iterations, X_to_add.shape[0])
    
    
    added_batch_size = int(X_to_add.shape[0]*1.0/X.shape[0]*batch_size + 0.5)
...
#     model, count = model_update_standard_lib_stochastic(batch_size, max_epoch, update_X, update_Y, alpha, error, model)
    t1 = time.time()
    model, count, para_list_all_epochs, gradient_list_all_epochs = model_update_standard_lib(max_epoch, X, Y, error, model, beta, random_ids_multi_super_iterations, batch_size, learning_rate_all_epochs, added_random_ids_multi_super_iteration, added_batch_size, X_to_add, Y_to_add)
